        ...
        <div className="p-4 text-extreme relative min-h-dvh">
            <div className="sticky top-0 z-20 bg-background py-4 flex items-center justify-between">
                ...
                <div className="bg-foreground px-4 py-2 border border-border rounded-lg">
                    <Link href={'/menu/cart'}>
                        <h2 className="text-lg">{data?.filter(menu => idInCart.includes(menu.id)).reduce((acc, value) => acc + (value.properties.Price.number * (cart ?? {})[value.id]), 0).toLocaleString()} THB</h2>
            ...
            <div className="space-y-4 mt-8 pb-40">
                {
                    isLoading
                        ? <MenuSkeleton />
                        : data && data.filter(dish => dish.properties.Category.select.name === category).map(dish => {
                            const dishName = dish.properties.Name.title[0].plain_text;
                            const imageUrl = dish.properties.Image.files?.[0]?.file.url;
                            const english = dish.properties.Description.rich_text.at(0)?.plain_text;
